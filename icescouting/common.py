# Common input & data reading routines for the Displaced Dimuon Scouting analysis
# Based off icedqcd/common.py
...
from icescouting import analytic
...
# GLOBALS
from configs.scouting.cuts import *
from configs.scouting.filter import *
...
def splitfactor(x, y, w, ids, args, skip_graph=True, use_dequantize=True):
    ...
    # -------------------------------------------------------------------------
    ## ** Add variables **
    print(f"Adding custom variables ...")
    # SV_mass
    data.x['SV', 'mass'] = analytic.get_sv_mass(muons=data.x['Muon'], svs=data.x['SV'])
    ...
    # -------------------------------------------------------------------------
    ## Add variables introduced in beginning of splitfactor
    jagged_vars.append('SV_mass')
    sv_vars.append('SV_mass')
    ...
    if not skip_graph:
        ...
        node_features = {'muon': muon_vars, 'sv': sv_vars}
